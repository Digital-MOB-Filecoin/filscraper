...
const { NovaApi } = require("./nova-api");
...
let filecoinChainInfoInfura = new FilecoinChainInfo(
  config.lotus.api_infura,
  config.lotus.token,
);
let zeroLabsClient = new ZeroLabsClient(
  config.scraper.renewable_energy_api,
  config.scraper.renewable_energy_token,
);
...
let wt = new WT();
let novaApi = new NovaApi();
let lilyClient = new LilyClient(
  config.lily.api,
  config.lily.token,
  config.lily.start_date,
);
  ...
  if (miner_sectors.has(miner)) {
    ...
    if (!sector_size) {
      ...
      if (minerInfo?.data?.result?.SectorSize) {
        ...
        ERROR(
          `[GetSectorSize] miner: ${miner} lotus.StateMinerInfo:  ${JSON.stringify(minerInfo?.data)}`,
        );
      }
    }
...
async function process_messages(block, messages) {
  ...
  while (messagesSlice.length) {
    await Promise.all(
      messagesSlice.splice(0, SCRAPE_LIMIT).map(async (msg) => {
        if (msg.ExitCode == 0 && msg.Params && msg.To.startsWith("f0")) {
          ...
          try {
            ...
            if (msg.Params != "null") {
              ERROR(
                `[ProcessMessages] error cbor.decode[${msg.Params}] : ${error}`,
              );
            }
          }
          ...
          if (decoded_params.length > 0) {
            switch (msg.Method) {
              case MinerMethods.PreCommitSector:
                {
                  ...
                  if (preCommitSector.DealIDs?.length == 0) {
                    ...
                    let miner_event = get_miner_events(
                      miner_events,
                      miner,
                      msg.Block,
                    );
                    miner_event.activated++;
                    miner_event.commited = miner_event.commited.add(
                      new BN(sector_size),
                    );
                    ...
                    miner_event.activated++;
                    miner_event.used = miner_event.used.add(
                      new BN(sector_size),
                    );
                    miner_events.set(miner, miner_event);
                  }
              ...
              case MinerMethods.TerminateSectors:
                {
                  let sectors = decode_sectors(
                    Buffer.from(decoded_params[0][0][2]),
                  );
              ...
              case MinerMethods.DeclareFaultsRecovered:
                {
                  let sectors;
                  try {
                    sectors = decode_sectors(
                      Buffer.from(decoded_params[0][0][2]),
                    );
                  } catch (e) {
                    ERROR(
                      "[ProcessMessages] MinerMethods.DeclareFaultsRecovered decode error",
                    );
  ...
  INFO(
    `[ProcessBlock] ${block} sectors: ${sectors.length} , sector events: ${sectors_events.length} , miner events: ${miner_events.size} , deals ${deals.length}`,
  );
...
async function scrape(reprocess, check_for_missing_blocks) {
  ...
  while (blocksSlice.length) {
    await Promise.all(
      blocksSlice.splice(0, scrape_limit).map(async (block) => {
        try {
          await scrape_block(
            block,
            "ScrapeBlock",
            check_for_missing_blocks,
            reprocess,
          );
        } catch (error) {
          ERROR(`[Scrape] error :`);
...
async function rescrape_missing_blocks(reprocess) {
  ...
  INFO(
    `[RescrapeMissingBlocks] total missing blocks: ${missing_blocks.length}`,
  );

  var blocksSlice = missing_blocks;
  while (blocksSlice.length) {
    await Promise.all(
      blocksSlice.splice(0, SCRAPE_LIMIT).map(async (item) => {
        try {
          await scrape_block(
            parseInt(item.missing_block),
            "RescrapeMissingBlock",
            true,
            reprocess,
          );
        } catch (error) {
...
async function rescrape_missing_blocks_from_filinfo() {
  ...
  INFO(
    `[RescrapeMissingBlockFromFilinfo] total missing blocks: ${missing_blocks.length}`,
  );

  var blocksSlice = missing_blocks;
  while (blocksSlice.length) {
    await Promise.all(
      blocksSlice.splice(0, INFURA_SCRAPE_LIMIT).map(async (item) => {
        try {
          await scrape_block_from_filinfo(
            parseInt(item.missing_block),
            "RescrapeMissingBlockFromFilinfo",
          );
        } catch (error) {
...
async function rescrape_msg_cid() {
  ...
  INFO(
    `[RescrapeMsgCid] total blocks with missing cid: ${blocks_with_missing_cid.length}`,
  );
    ...
    await Promise.all(
      blocksSlice.splice(0, SCRAPE_LIMIT).map(async (item) => {
        try {
          ...
          const result = await filecoinChainInfo.GetBlockMessagesByTipSet(
            block,
            tipSetKey,
          );
          if (result) {
          ...
          ERROR(
            `[RescrapeMsgCid] ${item.block_with_missing_cid} error :`,
            error,
          );
        }
      }),
...
async function rescrape_msg_cid_filplus() {
  ...
  INFO(
    `[RescrapeMsgCidFilPlus] total blocks with missing cid: ${blocks_with_missing_cid.length}`,
  );
      ...
      blocksSlice.splice(0, SCRAPE_LIMIT).map(async (item) => {
        try {
          ...
          if (result) {
            ...
            INFO(
              `[RescrapeMsgCidFilPlus] ${block} error: ${JSON.stringify(result)}`,
            );
          }
        } catch (error) {
          ERROR(
            `[RescrapeMsgCidFilPlus] ${item.block_with_missing_cid} error :`,
            error,
          );
        }
      }),
    ...
    if (minersResponse?.status == 200 && minersResponse?.data) {
      ...
      if (miners?.length > 0) {
        ...
        for (const miner of miners) {
          let transactionsResponse = await zeroLabsClient.GetTransactions(
            miner.id,
          );
          ...
          if (
            ...
            let minerData = {
              ...miner,
              recsTotal: transactionsResponse.data?.recsTotal,
            };

            INFO(
              `[UpdateRenewableEnergy] for ${miner.id} , transactions: ${transactions.length} , contracts: ${contracts.length}`,
            );
            ...
            if (transactions?.length) {
              for (const transaction of transactions) {
                ...
                await db.save_renewable_energy_from_transactions(
                  transactionData,
                );
...
async function refresh_views() {
  ...
  await db.refresh_miners_energy_re_share_view();
...
const mainLoop = async (_) => {
  ...
  let last_update_emissions = config.scraper.skip_updating_views_on_cold_start
    ? Date.now()
    : 0;
