CREATE MATERIALIZED VIEW IF NOT EXISTS fil_emissions_view_v8
AS
with data as (SELECT
       ...
       fil_emission_factors.value as ef_value
FROM fil_location_view
    LEFT JOIN fil_un_view ON fil_location_view.country = fil_un_view.country
    LEFT JOIN fil_emission_factors ON fil_location_view.country = fil_emission_factors.country),
    dates as (SELECT date_trunc('day', dd)::date as date FROM generate_series('2020-08-25'::timestamp, NOW()::timestamp, '1 day'::interval) dd),
    data_points as ( SELECT data.miner, data.ba, data.country, data.region, data.city, data.locations, data.un_value, data.ef_value, dates.date FROM data CROSS JOIN dates),
    ...
    SELECT
           ...
           capacity_data.date
    FROM capacity_data
    GROUP BY miner, locations, country, region, city, wt_value, un_value, ef_value, date, total_per_day, total
WITH DATA;

CREATE UNIQUE INDEX IF NOT EXISTS idx_fil_emissions_view_v8 ON fil_emissions_view_v8(miner, date, country, region, city);