CREATE MATERIALIZED VIEW IF NOT EXISTS fil_miners_green_scores
AS
select COALESCE(e.date, c.date)                                              AS date,
       COALESCE(e.miner, c.miner)                                            as miner,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_miners_green_scores ON fil_miners_green_scores(date, miner);