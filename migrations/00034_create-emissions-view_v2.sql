CREATE MATERIALIZED VIEW IF NOT EXISTS fil_emissions_view_v2
AS
with data as (SELECT
       miner,
       ba,
...
FROM fil_location_view
    ...
    GROUP BY miner,locations,date),
     capacity_data as (SELECT emissions_data.miner,
                              emissions_data.date,
                              (COALESCE(fil_miner_view_days_v4.total_per_day, 0) / locations) as total_per_day
                        FROM emissions_data
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_fil_emissions_view_v2 ON fil_emissions_view_v2(miner, date);