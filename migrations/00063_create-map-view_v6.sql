CREATE MATERIALIZED VIEW IF NOT EXISTS fil_map_view_v6
AS
with emissions_data as (SELECT
            ...
            FROM (
                SELECT
                    country,
                    date,
                    greatest(0,SUM((energy_use_kW_estimate - renewable_energy_kW) * CAST(COALESCE(avg_ef_value, 436) AS decimal) / 1000)) as cumulative_emissions_estimate,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_fil_map_view_v6 ON fil_map_view_v6(country);