CREATE MATERIALIZED VIEW IF NOT EXISTS fil_emissions_view_v5
AS
with data as (SELECT
...
FROM fil_location_view
    ...
    SELECT
           ...
           sum(ef_value) / locations as avg_ef_value,
           capacity_data.date
    FROM capacity_data
    GROUP BY miner, locations, country, region, city, date, total_per_day, total
WITH DATA;

CREATE UNIQUE INDEX IF NOT EXISTS idx_fil_emissions_view_v5 ON fil_emissions_view_v5(miner, date, country, region, city);